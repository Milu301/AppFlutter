...
import 'route_form_sheet.dart';

class AdminRoutesTab extends ConsumerStatefulWidget {
  final String adminId;
  const AdminRoutesTab({super.key, required this.adminId});

  @override
  ConsumerState<AdminRoutesTab> createState() => _AdminRoutesTabState();
}

class _AdminRoutesTabState extends ConsumerState<AdminRoutesTab> {
  ...
  Future<void> _load() async {
    ...
    try {
      final list = await ref.read(apiProvider).getAdminRoutes(
            widget.adminId,
      ...
      setState(() {
        error = ApiError(code: 'INTERNAL_ERROR', message: 'No se pudo cargar rutas: $msg');
        loading = false;
      });
    }
  }
  ...
  Future<void> _create() async {
    final payload = await showRouteFormSheet(context, title: 'Crear ruta');
    if (payload == null) return;

    try {
      final api = ref.read(apiProvider);
      final body = await api.createRoute(widget.adminId, payload);
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Ruta creada ✅')));
      await _load();
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _edit(Map<String, dynamic> r0) async {
    final payload = await showRouteFormSheet(context, title: 'Editar ruta', initial: r0);
    if (payload == null) return;

    final id = (r0['id'] ?? '').toString();
    if (id.isEmpty) return;

    try {
      final api = ref.read(apiProvider);
      final body = await api.updateRoute(id, payload);
      final err = api.extractError(body);
      if (err != null) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${err.message} (${err.code})')));
  ...
  Future<void> _toggle(Map<String, dynamic> r0) async {
    final id = (r0['id'] ?? '').toString();
    if (id.isEmpty) return;

    final current = (r0['status'] ?? 'active').toString();
    final next = current == 'active' ? 'inactive' : 'active';

    try {
      final api = ref.read(apiProvider);
      final body = await api.updateRoute(id, {'status': next});
      final err = api.extractError(body);
      if (err != null) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${err.message} (${err.code})')));
  ...
  Future<void> _delete(Map<String, dynamic> r0) async {
    final id = (r0['id'] ?? '').toString();
    if (id.isEmpty) return;
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('Soft delete: quedará inactiva (deleted_at). ¿Continuar?'),
        actions: [
    ...
    try {
      final api = ref.read(apiProvider);
      final body = await api.deleteRoute(id);
      final err = api.extractError(body);
      if (err != null) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${err.message} (${err.code})')));
  ...
  Future<void> _open(Map<String, dynamic> r0) async {
    final id = (r0['id'] ?? '').toString();
    if (id.isEmpty) return;
  ...
  Future<void> _actions(Map<String, dynamic> r0) async {
    ...
    if (selected == 'detail') _open(r0);
    if (selected == 'edit') _edit(r0);
    if (selected == 'toggle') _toggle(r0);
    if (selected == 'delete') _delete(r0);
  }
    ...
    return GlassCard(
      child: Column(
        children: [
          Row(
            children: [
              Text('Rutas', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w900)),
              const Spacer(),
          ...
          TextField(
            controller: qCtrl,
            decoration: const InputDecoration(prefixIcon: Icon(Icons.search), labelText: 'Buscar ruta'),
          ),
          const SizedBox(height: 10),
          Row(
            children: [
        ...
        Expanded(
          child: loading
              ...
              : (error != null)
                  ...
                  : (items.isEmpty)
                      ? EmptyView(title: 'Sin rutas', subtitle: 'Crea tu primera ruta.', onRetry: _load)
                      : ListView.separated(
                          itemCount: items.length,
                          separatorBuilder: (_, __) => const SizedBox(height: 10),
                          itemBuilder: (context, i) {
                            final r0 = items[i];
                            final name = (r0['name'] ?? 'Ruta').toString();
                            final st = (r0['status'] ?? 'active').toString();
                            // ✅ Compat: backend usa description; UI vieja usa notes
                            final notes = (r0['notes'] ?? r0['description'] ?? '').toString();

                            return InkWell(
                              onTap: () => _open(r0),
                              onLongPress: () => _actions(r0),
